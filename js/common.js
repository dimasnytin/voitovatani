...
const buttons = gsap.utils.toArray('section, footer, .banner');
...
buttons.forEach((btn) => {
  gsap.from(btn, {
    scrollTrigger: {
      ...
      trigger: btn,
      onEnter() {
        btn.classList.add('section-animation');
      },
      onLeave() {
...
var whatYouGet = new Swiper('.what-you-get .img-slider', {
    slidesPerView: 1,
    speed: 800,
    grabCursor: true,
    scrollbar: {
        el: '.nav-slider-scroll',
        draggable: true,
        dragSize: 70
    },
});
...
setTimeout(function(){
    let a = document.querySelector('.nav-slider .swiper-wrapper')
    console.log(getMatrix(a).x)
    // $()
    $('.nav-slider .swiper-wrapper').css({
        "margin-left" : `${(getMatrix(a).x - 5) * -1}px`
    })
}, 300)

let tm = 0;

whatYouGet.on('slideChange', function (e) {


    $('.slider-nav li').removeClass('active')
    $('.slider-nav li').eq(whatYouGet.activeIndex).addClass('active')

    $('swiper-scrollbar').addClass(`li-${whatYouGet.activeIndex}`)

    tm = whatYouGet.activeIndex
});

var navSlider = new Swiper('.nav-slider .swiper-container', {
    slidesPerView: 'auto',
    speed: 800,
    grabCursor: true,
    spaceBetween: 30,
...
if(window.matchMedia("(max-width: 901px)").matches) {
    navSlider.controller.control = this.whatYouGet;
    this.whatYouGet.controller.control = navSlider;
}




// this.whatYouGet.slideTo(3, 300)

let listBtn = document.querySelectorAll('.slider-nav li');

listBtn.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        this.whatYouGet.slideTo(index, 500)
    })
})
...
$('.slider-nav li').click(function () {
    ...
    $('.li-swiper-scrollbar').removeClass( `li-${tmpIndex}`)
    $('.li-swiper-scrollbar').addClass( `li-${index}`)
    tmpIndex = index;

})


var studentsWork = new Swiper('.works .swiper-container', {
    slidesPerView: 'auto',
    spaceBetween: 20,
    speed: 600,
    grabCursor: true,
...
var feedBack = new Swiper('.review-slider', {
    slidesPerView: 'auto',
    spaceBetween: 40,
    speed: 600,
    grabCursor: true,
...
var videoreviewSlider = new Swiper('.videoreview-slider', {
    slidesPerView: 3,
    speed: 600,
    grabCursor: true,
    scrollbar: {
        el: '.videoreview .swiper-scrollbar',
        draggable: true,
    },
    breakpoints: {
        320: {
...
$(".question-wrapper > .question.is-active").children(".answer").slideDown(300), $(".question-wrapper > .question").click(function () {
    $(this).toggleClass("is-active").children(".answer").slideToggle("ease-out")
}), $(".question-wrapper > .question").click(function () {
    $(this).siblings(".question").removeClass("is-active").children(".answer").slideUp(400)
    $(this).addClass('dsb')
    setTimeout(function () {
...
$(".header-wrapper, .footer__list-1, .btn-wrap, .prefooter, .menu-content  nav").on("click", "a", function (d) {
    d.preventDefault();
...
$(".mobileMenu .close, .mobileMenu ul li, .mobileMenu .link-st-1, .shedule-link, .buy-ticket").click(function () {
...
var rotateEl = gsap.utils.toArray('.speaker-circle, .stw')

rotateEl.forEach(rotateEl => {
    var rotate = gsap.to(rotateEl, {
        rotation: -360,
        duration: 30,
        onReverseComplete() {
            this.totalTime(30 * 100); // loop in reverse
        },
        repeat: -1,
        ease: "none",
    });
    ...
    ScrollTrigger.create({
        onUpdate: (self) => {
            ...
            progress = Math.floor((scrollTop / contentHeight) * 100) * rounds;

            rotate.timeScale(clamp(self.getVelocity() / 30));
            gsap.to(rotate, { timeScale: self.direction, duration: 0.5, overwrite: true, ease: "power1.inOut" });
        }
    });

    // make the right edge "stick" to the scroll bar. force3D: true improves performance
    gsap.set(rotateEl, { transformOrigin: "center center", force3D: true });
...
tickerItems.forEach(tickerItems => {
    var rotate = gsap.to(tickerItems, {
        xPercent: -100,
        duration: 50,
        onReverseComplete() {
            this.totalTime(30 * 100); // loop in reverse
    ...
    ScrollTrigger.create({
        onUpdate: (self) => {
            ...
            let cl = clamp(self.getVelocity() / 100);
            rotate.timeScale(cl < 0 ? cl * -1 : cl);

            
            
            gsap.to(rotate, { timeScale: 1, duration: 0.5, overwrite: true, ease: "power1.inOut" });
        }
    });

    // make the right edge "stick" to the scroll bar. force3D: true improves performance
    gsap.set(tickerItems, { transformOrigin: "center center", force3D: true });
...
const textAnimation1 = gsap.utils.toArray(".text-anim-1");
tween = gsap.to(textAnimation1, {
	xPercent: 20,
	ease: "none",
	scrollTrigger: {
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation1.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
});
const textAnimation2 = gsap.utils.toArray(".text-anim-2");
tween = gsap.to(textAnimation2, {
	xPercent: -20,
	ease: "none",
	scrollTrigger: {
		trigger: ".text-animation",
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation2.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
});
const textAnimation3 = gsap.utils.toArray(".text-anim-3");
tween = gsap.to(textAnimation3, {
	xPercent: 20,
	ease: "none",
	scrollTrigger: {
		trigger: ".text-animation",
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation3.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
...
const textAnimation4 = gsap.utils.toArray(".text-anim-4");
tween = gsap.to(textAnimation4, {
	xPercent: 20,
	ease: "none",
	scrollTrigger: {
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation4.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
});
const textAnimation5 = gsap.utils.toArray(".text-anim-5");
tween = gsap.to(textAnimation5, {
	xPercent: -20,
	ease: "none",
	scrollTrigger: {
		trigger: ".text-animation-2",
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation5.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
});
const textAnimation6 = gsap.utils.toArray(".text-anim-6");
tween = gsap.to(textAnimation6, {
	xPercent: 20,
	ease: "none",
	scrollTrigger: {
		trigger: ".text-animation-2",
		...
		scrub: 1,
		snap: {
			snapTo: 1 / (textAnimation6.length - 1),
			inertia: false,
			duration: {min: 0.1, max: 0.1}
		}
	}
...
// shdeuler-card-dropdown

$('.shdedule-card__button').click(function(){
    $(this).parent().find('.shdeuler-card-dropdown').slideToggle()
